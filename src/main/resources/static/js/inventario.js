...
const corpoTabela = document.querySelector(".tabela-inventario tbody");
const textoOrientacao = document.getElementById("ajudaFiltros");
...
function formatarLocalizacao(localizacao) {
    ...
    return nomes[localizacao] || localizacao || "--------";
}
...
function criarLinhaTabela(ferramenta) {
    const status = obterStatusFerramenta(ferramenta);

    const motivo = ferramenta.motivoDesativacao
        ? escaparHTML(ferramenta.motivoDesativacao)
        : '<span class="sem-dado">--------</span>';

    const nome = escaparHTML(ferramenta.nome);
    const localizacao = escaparHTML(
        formatarLocalizacao(ferramenta.localizacao)
    );
...
function mostrarMensagemTabela(mensagem) {
    corpoTabela.innerHTML = `
        <tr class="linha-sem-resultados">
            <td colspan="7">${escaparHTML(mensagem)}</td>
        </tr>
    `;
}
...
function renderizarTabela(ferramentas) {
    ...
    if (ferramentas.length === 0) {
        mostrarMensagemTabela("Nenhuma ferramenta encontrada.");
    ...
    atualizarTextoQuantidade(ferramentas.length);
}
...
async function carregarFerramentas() {
    textoOrientacao.textContent = "Carregando ferramentas...";
    mostrarMensagemTabela("Carregando ferramentas...");
    ...
    try {
        ...
        ferramentasCarregadas = Array.isArray(dados)
            ? dados
            : [];

        renderizarTabela(ferramentasCarregadas);
    } catch (erro) {
        console.error(erro);
...
function filtrarTabela() {
    ...
    const nomePesquisado = normalizarTexto(campoNome.value);
    const statusSelecionado = campoStatus.value;
    const localizacaoSelecionada = campoLocalizacao.value;

    const ferramentasFiltradas = ferramentasCarregadas.filter(
        function (ferramenta) {
            const status = obterStatusFerramenta(ferramenta);

            const nomeCorresponde =
                nomePesquisado === "" ||
                normalizarTexto(ferramenta.nome)
                    .includes(nomePesquisado);

            const statusCorresponde =
                statusSelecionado === "" ||
                status.valor === statusSelecionado;

            const localizacaoCorresponde =
                localizacaoSelecionada === "" ||
                ferramenta.localizacao === localizacaoSelecionada;
    ...
    renderizarTabela(ferramentasFiltradas);
}
...
botaoLimpar.addEventListener("click", function () {
    ...
    renderizarTabela(ferramentasCarregadas);
    textoOrientacao.textContent = textoOrientacaoOriginal;
